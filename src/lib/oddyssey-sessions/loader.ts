...
/**
 * Channel mix — splits gross revenue between direct sales (box office
 * + pre-sale) and third-party resellers (hotels, OTAs, Fever-style
 * platforms). Third-party rows are identified by a naming convention
 * used in Ticketure's ticket group config: any group containing
 * "third party" or "commission" — e.g., "Third Party Sales - 20% Commission".
 * The commission rate is parsed from the name when present so we can
 * calculate actual revenue flowing to Oddyssey.
 */
export interface ChannelMix {
  ...
  // Weighted avg commission rate across third-party lines when names
  // include a rate; null if we couldn't parse any.
  commission_rate: number | null; // 0-1
  commission_amount: number; // estimated dollars owed to third parties
  net_to_oddyssey: number; // direct + third_party × (1 - commission_rate)
  third_party_lines: {
    name: string;
    gross: number;
    rate: number | null;
