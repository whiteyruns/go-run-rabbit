...
function buildInputForDate(date?: string) {
  let csv: string;
  try { csv = fs.readFileSync(LATEST_CSV, "utf-8"); } catch { return null; }
  ...
  const notes = loadNotes("noir", summary.date);
  ...
  return {
    ...
    fallback_tickets: summary.tickets_sold,
    fallback_revenue: summary.revenue,
    dashboard_url: "https://gorunrabbit.com/oddyssey-manor/admin/noir/summary",
