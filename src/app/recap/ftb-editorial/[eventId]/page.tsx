...
export default function EditorialRecapPage({
  params,
}: {
  ...
  return (
    <AuthGate>
      <RecapBody eventId={params.eventId} />
    </AuthGate>
  );
}

// ============================================================================
// Auth gate — cream editorial styling, not FTB dark
// ============================================================================
...
function AuthGate({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center recap-no-print px-8" style={{ background: "#fdf9f3" }}>
      <form onSubmit={submit} className="flex flex-col items-center gap-8 max-w-sm w-full">
        <p className="serif text-4xl font-bold tracking-tighter text-[#1c1c18]">FEED THE BLOCK</p>
...
function RecapBody({ eventId }: { eventId: string }) {
  ...
  const dwellHrs = (card.dwellMinutes / 60).toFixed(1);
  const topHotels = [...ds.hotels]
      ...
      <main className="pt-20 md:pt-24">
        ...
        <section className="min-h-[90vh] px-8 md:px-20 py-20 md:py-0 flex flex-col justify-center border-b-[0.5px] border-[#c9912b]/30">
          <div className="max-w-7xl mx-auto w-full">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12 mt-10">
              <StatRule value={fmtNum(card.visits)} label="Total Visitors" />
              <StatRule
                value={`${dwellHrs} Hrs`}
                label="Avg. Dwell Time"
              />
              <StatRule value={card.yoy} label="Visits Year-over-Year" />
              <StatRule value={fmt(card.estimatedImpact)} label="Direct Economic Spend" />
            </div>
          </div>
        </section>
          ...
          <div className="max-w-7xl mx-auto px-8 md:px-20 mt-12 md:mt-16 grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-12">
            <div className="md:col-span-4">
              <div className="flex flex-col gap-6">
                ...
                <EventFact label="Day" value={event.eventDay} />
                <EventFact label="Producer" value="Wynn Nightlife × Corner Bar Management" />
              </div>
            </div>
            <div className="md:col-span-8">
              <p className="text-[17px] text-[#1c1c18]/80 leading-loose">
                The air was electric from first note. Unlike traditional nightlife
                activations, Feed The Block prioritizes the pedestrian experience, utilizing
                the existing architectural bones of Fremont East to create a 360-degree
                sensory environment. The collaboration between Wynn Nightlife&rsquo;s
                production expertise and Corner Bar Management&rsquo;s local soul manifested
                in a night that recalibrated what Downtown entertainment can achieve — and
                the Placer.ai data confirms what the sidewalks already knew.
              </p>
            </div>
          </div>
        </section>
                    ...
                    <div className="flex flex-wrap gap-3 items-center">
                      {artist.hits.slice(0, 5).map((h, i) => (
                        <span key={h.title} className="flex items-center gap-3">
                          <span className="serif italic text-base">&ldquo;{h.title}&rdquo;</span>
        ...
        <section className="py-24 md:py-32 px-8 md:px-20 bg-[#ebe8e2] recap-page-break">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-20 items-end">
              ...
              <div className="md:col-span-7 relative pt-20">
                <div className="flex items-end gap-4 md:gap-6 h-64 border-b border-[#1c1c18]/20 relative">
                  {topHotels.map((h, i) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`${isFirst ? "bg-[#7f5700]" : "bg-[#7f5700]"} absolute bottom-0 left-0 right-0`}
                          style={{
                            height: `${pct}%`,
                            opacity: isFirst ? 1 : 0.65 - i * 0.15,
                          }}
                        />
                      </div>
                    );
                ...
                <div className="flex gap-4 md:gap-6 mt-3">
                  {topHotels.map((h) => (
                    <div key={h.id} className="flex-1 uppercase tracking-widest text-[9px] text-[#1c1c18]/60 text-center">
            ...
            {/* Demographic editorial paragraph */}
            <div className="mt-20 max-w-4xl">
              <p className="serif italic text-2xl md:text-3xl text-[#1c1c18]/80 leading-snug">
                ...
                , and is{" "}
                <span className="text-[#7f5700] font-bold not-italic">34% Hispanic or Latino</span>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-12 text-left">
              ...
              <ImpactBlock
                ...
                caption={`${fmtNum(Math.round(card.hotelCrossover * 0.5))} crossover visitors × 1 drink @ $12.`}
              />
              <ImpactBlock
                title="Gaming Revenue"
                value={fmt(card.hotelCrossover * 0.1 * 50)}
                caption={`${fmtNum(Math.round(card.hotelCrossover * 0.1))} players × $50 average gaming volume.`}
              />
            </div>
            <p className="serif italic text-sm text-[#1c1c18]/50 mt-16 max-w-xl mx-auto">
              Model excludes dining, retail, rideshare, and ancillary spend — true
              impact materially higher.
            </p>
          </div>
        </section>
        ...
        <footer className="w-full flex flex-col md:flex-row justify-between items-center gap-6 bg-[#fdf9f3] px-8 md:px-12 py-12 border-t border-[#c9912b]">
          ...
          <div className="serif uppercase tracking-widest text-[9px] text-[#1c1c18]/50 text-center">
            Post-event recap · {event.eventDay}, {event.eventDate} · Placer.ai verified
          </div>
